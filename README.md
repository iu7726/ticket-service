...
> **결론:** 3,000명의 동시 접속 상황에서 **재고 정합성 100% 유지 성공.**

| 지표                         | 결과값    | 의미                                         |
| :--------------------------- | :-------- | :------------------------------------------- |
| **총 요청 수 (VUs)**         | 3,000명   | 짧은 시간(약 8초) 동안 집중된 트래픽         |
| **판매 성공 (Success)**      | **100건** | `final_success_count` (정확히 100개 판매)    |
| **판매 실패 (Sold Out)**     | 2,900건   | 재고 소진 후 들어온 요청은 정상적으로 거절됨 |
| **오버 셀링 (Over-selling)** | **0건**   | Redis Lua Script를 통한 원자성 보장 확인     |
...
█ TOTAL RESULTS
   ...
   checks_succeeded...: 100.00% 100 out of 100  (예약된 100명 전원 결제 확정 성공)

   CUSTOM
   final_success_count............: 100   12.67/s  (🏆 최종 판매 완료: 100건)
   reserve_success_count..........: 100   12.67/s  (예약 성공: 100건)
   error_count....................: 2900  367.50/s (재고 소진으로 인한 거절)

   HTTP
   http_req_duration..............: p(95)=6.3s     (평균 응답 지연)
```
